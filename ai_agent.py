...
class AiAgent:
    ...
    def __init__(self, config: Config, device_tracker: Optional['DeviceStateTracker'] = None, context=None):
        self.config = config
        self.device_tracker = device_tracker
        self.prompt_builder = PromptBuilder(config)
        self.context = context  # RuntimeContext for dependency injection
    ...
    def _announce_ai_action(self, topic: str, payload: str) -> None:
        ...
        try:
            tools.send_mqtt_message(announce_topic, json.dumps(announcement), context=self.context)
    ...
    def execute_tool_call(self, tool_name: str, arguments: dict) -> str:
        ...
        if tool_name == "send_mqtt_message":
            ...
            try:
                return tools.send_mqtt_message(topic, payload, context=self.context)
            except Exception as e:  # pylint: disable=broad-exception-caught
        ...
        tool_map = {
            ...
            "raise_alert": lambda args: self.raise_alert(
    ...
    def raise_alert(self, severity: float, reason: str, context: dict = None) -> str:
        ...
        if severity < 0.7:
            ...
            try:
                tools.send_mqtt_message("mqtt2ai/alerts", json.dumps(notification), context=self.context)
    ...
    def _execute_alert_ai_call(self, prompt: str) -> None:
        ...
        try:
            ...
            # Using basic model config from main agent
    ...
    def _execute_openai_api_call(self, provider: str, prompt: str,
        ...
        if not OPENAI_AVAILABLE:
            logging.error("OpenAI SDK not installed. Run: pip install openai")
            return
        ...
        try:
            ...
            for iteration in range(max_iterations):
                ...
                if message.tool_calls:
                    ...
                    redundant_patterns = ["already exists", "No need to re-learn", "No update needed"]
                    redundant_count = 0
                    ...
                    if redundant_count == len(message.tool_calls) and redundant_count > 0:
                        logging.info("%s[Early Stop] All tool calls redundant%s", orange_bold, reset)
                        break
    ...
    def _execute_claude_sdk_call(self, provider: str, prompt: str,
        ...
        if not ANTHROPIC_AVAILABLE:
            logging.error("Anthropic SDK not installed.")
            return
