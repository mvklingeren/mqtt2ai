...
class TriggerResult:
    ...
    def __str__(self):
        if self.should_trigger:
            return f"TRIGGER: {self.reason} (field={self.field_name}, {self.old_value} -> {self.new_value})"
        return "NO_TRIGGER"
...
class TopicState:
    ...
    state_fields: dict = field(default_factory=dict)  # field_name -> last_value
    numeric_fields: dict = field(default_factory=dict)  # field_name -> NumericFieldState
...
class TriggerAnalyzer:
    ...
    def _load_config(self, config_path: str) -> dict:
        ...
        default_config = {
            ...
            "state_fields": ["contact", "smoke", "occupancy", "water_leak", "tamper", "motion", "state", "action"],
            "numeric_fields": {
    ...
    def _check_state_fields(self, topic_state: TopicState, payload: dict) -> Optional[TriggerResult]:
    ...
    def _check_numeric_fields(self, topic_state: TopicState, payload: dict) -> Optional[TriggerResult]:
        ...
        for field_name, thresholds in self.config["numeric_fields"].items():
            ...
            if immediate_change >= immediate_delta:
                ...
                return TriggerResult(
                    should_trigger=True,
                    reason=f"Immediate change in '{field_name}' (delta={immediate_change:.1f}, threshold={immediate_delta})",
                    field_name=field_name,
                    old_value=old_value,
                    new_value=new_value
                )
            ...
            if drift_change >= drift_delta:
                ...
                return TriggerResult(
                    should_trigger=True,
                    reason=f"Drift detected in '{field_name}' from baseline (drift={drift_change:.1f}, threshold={drift_delta})",
                    field_name=field_name,
    ...
    def analyze(self, topic: str, raw_payload: str) -> TriggerResult:
        ...
        if not isinstance(payload, dict):
            return TriggerResult(should_trigger=False, reason="Payload is not a JSON object")
        ...
        return TriggerResult(should_trigger=False, reason="No significant changes detected")
...
if __name__ == "__main__":
    ...
    test_messages = [
        ...
        ("zigbee2mqtt/power_plug_1", '{"power": 55, "state": "ON"}'),  # Small change, no trigger
        ("zigbee2mqtt/power_plug_1", '{"power": 200, "state": "ON"}'),  # Big jump, should trigger
        ...
        ("zigbee2mqtt/smoke_detector", '{"smoke": true, "temperature": 25}'),  # Smoke! Should trigger
    ]
    
    print("Simulating message sequence:\n")
    for topic, payload in test_messages:
        result = analyzer.analyze(topic, payload)
        status = "🔴 TRIGGER" if result.should_trigger else "⚪ no trigger"
